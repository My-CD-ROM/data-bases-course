...
колонку через `ALTER TABLE` (Лекція 18) або переставляє порядок при
...
що для великих обсягів даних (Лекція 17, імпорт CSV) помітно швидше за
...
`JOIN` (Лекція 8) з'єднує таблиці саме через збіг значення первинного
