# Лекція 21. Функції у PostgreSQL

*Подання (Лекція 20) чудово зберігають готовий `SELECT`, але не
...
колонкою (Лекція 18): `IF`/`ELSIF`/`ELSE` дозволяє виразити
...
(Лекція 15) — вона не може сама зробити `COMMIT` чи `ROLLBACK`;
