...
Немає потреби уявляти складну оптимізацію (про неї — Лекція 16, індекси).
...
Важливо розуміти: без `ORDER BY` (тема Лекції 12) порядок рядків у
...
групування (`GROUP BY`) і відбір груп (`HAVING`, Лекція 10) підпорядковані
