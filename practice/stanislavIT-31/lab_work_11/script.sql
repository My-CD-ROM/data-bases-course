...
-- ============================================================
-- Task 1. printf() on a numeric column
-- Raw value vs a formatted value with exactly two decimals and text.
-- ============================================================
SELECT id,
       purchase_price AS raw_price,
...
-- ============================================================
-- Task 2. printf() with a fixed field width / zero-padding
-- quantity right-aligned in a width-5 field, id zero-padded to 3 digits.
-- ============================================================
SELECT printf('Delivery #%03d', id) AS label,
       printf('%5d units', quantity) AS qty_align
FROM deliveries
ORDER BY id;

-- ============================================================
-- Task 3. Shift a date relative to an existing one
-- Payment deadline = 30 days after the delivery.
-- Practical sense: shows when the invoice must be settled to keep
-- the discount / avoid a penalty.
-- ============================================================
SELECT id, delivery_date,
       date(delivery_date, '+30 days') AS payment_deadline
FROM deliveries
ORDER BY id;

-- ============================================================
-- Task 4. strftime() and GROUP BY year (with month as well)
-- How many deliveries happened in each year / in each month.
-- ============================================================
SELECT strftime('%Y', delivery_date) AS yr,
       COUNT(*) AS deliveries_count
FROM deliveries
GROUP BY yr
ORDER BY yr;

-- deliveries grouped by year and month together
SELECT strftime('%Y', delivery_date) AS yr,
       strftime('%m', delivery_date) AS mo,
       COUNT(*) AS cnt
FROM deliveries
GROUP BY yr, mo
ORDER BY yr, mo;
...
-- SQLite does NOT raise an error — it silently returns NULL.
-- ============================================================
SELECT 'not-a-date' AS input,
       strftime('%Y', 'not-a-date') AS year_of_not_a_date,
       date('not-a-date') AS shifted;
