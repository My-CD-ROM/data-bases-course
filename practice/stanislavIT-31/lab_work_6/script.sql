...
-- The schema does NOT change — only selection queries over the
-- existing three tables (medicines, suppliers, deliveries).
--
...
-- ============================================================
-- Task 3 prep. Add a NEW dimension-table-1 row (a medicine) and a NEW
-- dimension-table-2 row (a supplier) that has NO delivery yet, so the
-- LEFT JOIN query in Task 3 returns at least one result.
-- ============================================================
INSERT INTO medicines (name, manufacturer, form, price, stock_quantity)
VALUES ('New probiotic', 'Probiotic Labs', 'capsules', 120.0, 25);

INSERT INTO suppliers (name, contact_person, phone, city)
VALUES ('FreshBio Distributor', 'Petro Savchenko', '+380****0099', 'Kyiv');

-- ============================================================
-- Task 1. INNER JOIN fact table + dimension 1
-- Show the medicine NAME instead of medicine_id, with ordering so the
-- result is meaningful (newest delivery first).
-- ============================================================
...
-- One result row shows three human-readable values at once (no ids).
-- ============================================================
...
-- ============================================================
-- Task 3. LEFT JOIN — dimension rows with NO pair in the fact table
-- Suppliers that have never made a delivery (LEFT JOIN + IS NULL).
-- The new "FreshBio Distributor" must appear here.
-- ============================================================
...
-- Count rows: dimension 1 x dimension 2 (comma syntax, no JOIN/WHERE)
-- MUST equal the PRODUCT of the individual counts, not their sum.
-- ============================================================
SELECT COUNT(*) AS medicines_count FROM medicines;
SELECT COUNT(*) AS suppliers_count FROM suppliers;
SELECT COUNT(*) AS cross_product FROM medicines, suppliers;

-- ============================================================
-- Task 5. Justification of each JOIN type (written in readme.md)
-- ============================================================
-- (No query needed; see readme.md for the reasoning.)
